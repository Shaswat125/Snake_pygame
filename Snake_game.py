...
def plot_snake(gameWindow, color, snk_list, sn_size):
    # print(snk_list)
    ##we display every element of the snk_list and append accordingly
    for a,b in snk_list:
...
while not exit_game:
    ...
    if abs(x - food_x) < 15 and abs(y - food_y) < 15:
        ...
        food_y = random.randint(10, height / 2)
        snk_length+=5#only when the snake eats food its size should be increased
    ...
    if len(snk_list)>snk_length:
        del snk_list[0]##only when the snake eats food its size should be increased
